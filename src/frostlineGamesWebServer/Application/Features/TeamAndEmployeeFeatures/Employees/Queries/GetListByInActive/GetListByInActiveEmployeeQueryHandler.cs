...
public class GetListByInActiveEmployeeQueryHandler : IRequestHandler<GetListByInActiveEmployeeQueryRequest, GetListResponse<GetListByInActiveEmployeeQueryResponse>>
{
    ...
    public GetListByInActiveEmployeeQueryHandler(IEmployeeService employeeService, IMapper mapper, EmployeeBusinessRules employeeBusinessRules, IUserDetailService userDetailService, IUserDetailImageFileRepository userDetailImageFileRepository, IUserService userService)
    {
    ...
    public async Task<GetListResponse<GetListByInActiveEmployeeQueryResponse>> Handle(GetListByInActiveEmployeeQueryRequest request, CancellationToken cancellationToken)
    {
        await _employeeBusinessRules.EmployeeShouldBeListedWhenSelected(request.PageRequest.Page, request.PageRequest.PageSize);

        IPaginate<Employee> employeeList = await _employeeService.GetInActiveList(request.PageRequest.Page, size: request.PageRequest.PageSize);
        ...
        GetListResponse<GetListByInActiveEmployeeQueryResponse> mappedList = _mapper.Map<GetListResponse<GetListByInActiveEmployeeQueryResponse>>(employeeList);
